...
## گوشی‌های Cisco 7942G

### داخلی ۲۰۱ — Khodayari(BD)

پیکربندی سمت Issabel برای گوشی جدید با MAC `20BBC09337D0` در تاریخ
`2026-08-24` ایجاد شده است:

- داخلی و نام تماس‌گیرنده: `201` و `Khodayari(BD)`
- فایل اختصاصی TFTP: `/tftpboot/SEP20BBC09337D0.cnf.xml`
- آدرس SIP و TFTP: `192.168.10.101`
- firmware انتخاب‌شده: `SIP42.8-5-4S`
- روش احراز هویت اولیه: SIP Digest با رمز منحصربه‌فرد (رمز در Git نگهداری نمی‌شود)

فایل از خود سرویس TFTP دریافت و با نسخهٔ روی دیسک تطبیق داده شده، peer داخلی
در Asterisk و مسیر `201@ext-local` نیز فعال است. تا وقتی گوشی به LAN/PoE متصل
نشده و فایل را دریافت نکرده باشد، وضعیت peer طبیعی است که `UNKNOWN` بماند.

برای راه‌اندازی فیزیکی، گوشی را از درگاه `SW` به شبکهٔ تلفن وصل کنید. اگر DHCP
گزینهٔ TFTP را ارائه نمی‌کند، بدون Factory Reset در Network Configuration،
`Alternate TFTP` را فعال و `TFTP Server 1` را روی `192.168.10.101` قرار دهید،
سپس گوشی را reboot کنید. ثبت موفق باید با دستور زیر دیده شود:

```bash
asterisk -rx "sip show peer 201"
# Addr->IP، Useragent و Reg. Contact باید مقدار داشته باشند و Status باید OK باشد.
```

در این مرحله نباید override بدون رمز برای ۲۰۱ ساخته شود. فقط اگر firmware بعد
از دریافت فایل TFTP نتوانست Digest را کامل کند، ابتدا IP گوشی را در DHCP برای
همین MAC رزرو کنید و سپس الگوی ACL محدود به `/32` را مانند داخلی‌های ۲۰۰ و ۲۲۲
اعمال کنید. استفاده از `insecure=port,invite` بدون IP ثابت و ACL ممنوع است.
