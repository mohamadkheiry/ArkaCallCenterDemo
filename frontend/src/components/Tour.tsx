...
const STEPS: Step[] = [
  ...
  },
  {
    target: 'voice',
    title: 'صدای گوینده',
    body: 'صدایی که هوش مصنوعی با آن صحبت می‌کند را از بین گوینده‌های موجود انتخاب کنید.',
    icon: Mic,
  },
  {
    target: 'calls',
...
export default function Tour({ open, isAdmin, onClose }: { open: boolean; isAdmin: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    // اگر عنصر خارج از دید بود (مثلاً سایدبار بسته در موبایل)، مرکز صفحه
    if (r.width === 0 || r.left >= window.innerWidth || r.right <= 0) return setRect(null)
    setRect(r)
  ...
  // موقعیت کارت: کنار عنصر هایلایت‌شده یا مرکز
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div style={cardStyle} className="w-[340px] max-w-[calc(100vw-32px)] animate-in rounded-2xl bg-white p-5 shadow-2xl">
