...
## محدودیتِ تماس (ضدِ سیلِ تماس)
اگر چند ایشو یک‌جا اساین شوند (مثلاً هنگام ساختِ انبوهِ ایشو)، نباید ده‌ها تماس گرفته شود.
دو سازوکار وجود دارد:

1. **ادغام (coalescing):** در هر اجرا، هر تعداد ایشوی جدید فقط **یک تماس** می‌گیرند —
   چون صوتِ پخش‌شده ثابت است (`arka/jira_check` = «چک کردن جیرا») و شماره‌ی ایشو را نمی‌گوید،
   یک تماس برای کلِ دسته کافی است. کلیدها در لاگ ثبت می‌شوند.
2. **بازه‌ی خنک‌شدن (cooldown):** بین دو تماسِ اطلاع‌رسانی برای هر کاربر حداقل
   `notify_cooldown_s` ثانیه فاصله است (پیش‌فرض **۶۰۰** = ۱۰ دقیقه). در این بازه، ایشوهای
   جدید «دیده‌شده» علامت می‌خورند و در لاگ `RATE-LIMITED` ثبت می‌شوند، ولی تماسی گرفته نمی‌شود
   (کاربر همین اواخر خبردار شده و با یک بار چک‌کردنِ جیرا همه را می‌بیند).

تنظیم: `notify_cooldown_s` در ریشه‌ی `config.json` (برای همه) یا داخلِ هر کاربر (اختصاصی).
وضعیتِ آخرین تماس در `state/user_<name>.json` → `last_notify_ts` نگهداری می‌شود.

> منطقِ **تکرارِ تماسِ بی‌جواب** (تا ۳ بار، هر ۵ دقیقه) دست‌نخورده است؛ فقط برای هر شماره
> حداکثر **یک** ورودیِ صف نگهداری می‌شود تا تکرارها هم سیل نشوند.
