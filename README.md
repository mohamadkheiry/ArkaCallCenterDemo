...
رابط بازطراحی‌شدهٔ داشبورد از همان پایگاه دانش متنی/فایلی و مسیر تماس پایدار استفاده می‌کند.
جزئیات اجزای رابط، تست و ادامه توسعه در [راهنمای فرانت‌اند](frontend/README.md) و
[سیستم طراحی داشبورد](docs/DASHBOARD_DESIGN.md) ثبت شده‌اند.
طراحی مستقل صفحه ورود، تصویرهای اختصاصی و قراردادهای OTP نیز در
[راهنمای صفحه ورود](docs/LOGIN_DESIGN.md) مستند شده‌اند.
...
- **پاسخ مستقیم دانشی:** سرور کل `RawText` تأییدشدهٔ همان کاربر را به قطعه‌های منبع با شناسه‌های `Sxxxxxx` تبدیل می‌کند و مدل Chat در هر سؤال پاسخ‌پذیر ۱ تا ۴ شناسه را در فیلد `evidenceIds` انتخاب می‌کند. سرور متن همان قطعه‌های منبع را به‌صورت صوتی می‌خواند؛ در مسیر تماس embedding یا Top-K RAG اجرا نمی‌شود.
- **حافظهٔ همان تماس:** حداکثر شش نوبت اخیر کاربر و دستیار برای فهم سؤال‌های پیرو، ضمیرها و انتخاب بین گزینه‌های قبلی نگه داشته می‌شود و با پایان تماس از بین می‌رود. تاریخچه فقط برای فهم منظور است؛ واقعیت پاسخ همچنان باید در نسخهٔ فعلی پایگاه دانش وجود داشته باشد و در نبود معیار شخصی، سیستم سؤال تکمیلی می‌پرسد.
