...
- **جریان هر تماس (`CallHandler`):**
  ...
  6. پس از پایان هر نوبت، تشخیص هویت کسب‌وکار و نیت‌های اجتماعی فارسی پیش از RAG اجرا می‌شود؛ سلام، احوال‌پرسی، تشکر، عذرخواهی، واکنش احساسی کوتاه، تأیید/رد و خداحافظی پاسخ مستقیم می‌گیرند و در سؤال‌های بی‌پاسخ ثبت نمی‌شوند. سؤال واقعی یا جمله ترکیبی سپس به `RagService` می‌رود تا فقط قطعه‌های مرتبط را بازیابی کند. query embedding حداکثر ۸ ثانیه فرصت دارد؛ در timeout یا خطای شبکه، بازیابی واژگانی BM25/شباهت فارسی با آستانه سخت‌گیرانه جایگزین می‌شود و `response.create` با context معتبر یا fallback قطعی ارسال خواهد شد.
