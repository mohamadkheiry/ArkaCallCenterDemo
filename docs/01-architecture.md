...
### ضبط هم‌زمان دو سمت مکالمه

- ورودی AudioSocket و خروجی AI هنگام رسیدن به شبکه مستقیماً پشت سر هم ذخیره نمی‌شوند؛ چون ترتیب رسیدن packetها با timeline شنیده‌شده یکسان نیست.
- پمپ خروجی هر ۲۰ میلی‌ثانیه دقیقاً همان فریم پخش‌شده را همراه فریم متناظر caller به `CallRecordingBuffer` می‌دهد و دو کانال PCM16 با saturation ترکیب می‌شوند.
- پردازش نهایی سکوت را به‌صورت فریم‌های ۲۰ میلی‌ثانیه‌ای و با RMS/peak تشخیص می‌دهد. فاصله‌های طولانی حداکثر به ۲۸۰ میلی‌ثانیه می‌رسند و هیچ noise-gate نمونه‌به‌نمونه‌ای روی گفتار اجرا نمی‌شود؛ بنابراین انتهای کلمات و صداهای کم‌حجم بریده نمی‌شوند.
