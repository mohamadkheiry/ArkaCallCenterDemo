...
سرویس `realtime` ابتدا نوبت‌های اجتماعی/کنترلی را بدون RAG پاسخ می‌دهد و برای سؤال واقعی Hybrid RAG را اجرا می‌کند. نتیجهٔ دانشی پنج حالت دارد: پاسخ دارای شاهد، مرتبط ولی بی‌پاسخ، خارج از حوزه، KB خالی و اختلال فنی. فقط «مرتبط ولی بی‌پاسخ/KB خالی» در گزارش unanswered ثبت و به کارشناس یا اپراتور ارجاع می‌شود؛ سؤال خارج از حوزه باید حوزهٔ مجموعه را معرفی کند. تنظیم قطع تماس پس از سکوت کامل:
...
مقدار `0` قطع خودکار را غیرفعال می‌کند. پس از انتشار حداقل چهار مسیر را جدا تست کنید: احوال‌پرسی، سؤال دارای پاسخ صریح در KB، سؤال مرتبطِ بی‌پاسخ و سؤال کاملاً خارج از حوزه. transcript، `AnsweredFromKb`، `UnansweredQuestionsJson` و log مسیر باید با نتیجهٔ مورد انتظار تطبیق داده شوند؛ شنیدن پیام خوش‌آمد به‌تنهایی قبولی AudioSocket نیست.
