...
هر بار که یکی از رویدادهای بالا رخ دهد، درخواست لید بدون بررسی تکراری‌بودن شماره یا مرحله به CRM ارسال می‌شود. بنابراین پاک‌سازی یا تغییر اطلاعات در CRM مقصد باعث نمی‌شود دیتابیس محلی ارسال بعدی را مسدود کند.

ترکیب شماره و مرحله در `CrmLeadSubmissions` یکتا باقی مانده است، اما این جدول فقط snapshot آخرین نتیجه، پیام پاسخ و زمان آخرین تلاش را برای عیب‌یابی نگه می‌دارد و برای deduplication استفاده نمی‌شود.
