...
- [x] **پالایش دوم صفحه ورود — ۲۰۲۶-۰۹-۰۶:** فقط CSS اختصاصی login؛ قاب سفید روی زمینه یاسی، توازن ستون‌ها، تایپوگرافی و کنترل‌های خواناتر، OTP و موبایل بهینه‌تر. تصویر و لوگوی فعلی حفظ شدند. هشت viewport در مرورگر داخلی، اعتبارسنجی، loading، خطای SMS/مسیر تماس، ۴۲۹ و شمارنده، paste فارسی، ورود کاربر جدید/موجود و focus صفحه‌کلید بررسی شدند؛ APIهای ورود شبیه‌سازی بودند و کد واقعی ارسال نشد. build و lint موفق (warning قدیمی `cn`). مرجع ۱۵۳۶×۱۰۲۴ و screenshot نهایی مقایسه شدند؛ دفتر تطبیق و محدودیت‌های QA در `docs/LOGIN_DESIGN.md` است. منطق ورود، پخش مکالمه، DB و تلفن تغییر ندارند.
- [x] **انتشار پالایش ورود — ۲۰۲۶-۰۹-۰۶:** commit `8110e65` ساعت `16:02:05 UTC` فقط روی web منتشر شد. image و StartedAt سه سرویس api/realtime/db با preflight همین انتشار یکسان ماندند. Nginx و کانتینر موقت، سلامت داخلی/عمومی، bundle جدید و نماهای دسکتاپ/موبایل عمومی در IAB تأیید شدند. بکاپ root-only، hashها و rollback بدون از دست دادن اصلاح پخش مکالمه در `docs/LAST_DEPLOYMENT.md` ثبت است؛ Docker و واحد برنامه enabled باقی ماندند.
- [x] **رفع خطای اولین پخش مکالمه — ۲۰۲۶-۰۹-۰۵:** خطا در Chrome بازتولید شد: cleanup وابسته به تغییر Blob URL، `play()` نخست را با `pause()` قطع می‌کرد (`AbortError`)؛ درخواست دوم همان WAV سالم را پخش می‌کرد. منابع پخش به ref و cleanup طول عمر پخش‌کننده منتقل شدند؛ قفل هم‌زمان، cancel دریافت در خروج و آزادسازی فایل خراب/قدیمی اضافه شد. هر ۱۷ بررسی مرورگر با WAV مصنوعی در تماس کاربر، پنل ادمین، سؤال بی‌پاسخ و موبایل موفق است؛ build و lint بدون error (همان warning قدیمی `cn`) هستند. API، فایل‌های ضبط‌شده و DB تغییر ندارند؛ قرارداد نگه‌داری در `frontend/README.md` است.
- [x] **انتشار اصلاح پخش — ۲۰۲۶-۰۹-۰۵:** commit `30eba38` ساعت `16:17:35 UTC` فقط روی web منتشر شد؛ همان ۱۷ تست با API شبیه‌سازی‌شده روی assetهای عمومی هم موفق بود. health برابر ۲۰۰ و صوت بدون token برابر ۴۰۱ است. api/realtime/db بدون تغییر Image ID یا StartedAt، فایل‌های ضبط‌شده دست‌نخورده و startup خودکار فعال باقی ماندند. backup و rollback در `docs/LAST_DEPLOYMENT.md` ثبت شده‌اند.
- [x] **بازطراحی مستقل ورود — ۲۰۲۶-۰۹-۰۵:** صفحه سفید/نیمه‌شب با تصویر اختصاصی تلفن، نسخه جمع‌وجور موبایل، فرم و OTP یکپارچه. `useLoginFlow` قفل هم‌زمان درخواست‌ها، cooldown مطلق، ارقام فارسی/عربی و +98 و فوکوس مجدد کد را کنترل می‌کند؛ API و سرویس‌ها تغییر نکردند. build/lint و ۲۶ بررسی مرورگر با API شبیه‌سازی‌شده در هشت اندازه موفق‌اند. قراردادها، assetها و تفاوت‌های عمدی در `docs/LOGIN_DESIGN.md` ثبت شده‌اند.
- [x] **انتشار ورود — ۲۰۲۶-۰۹-۰۵:** commit `ef08811` در ساعت ۱۵:۴۴ UTC فقط روی web منتشر شد. ۲۶ بررسی UI روی assetهای عمومی تکرار شد؛ لوگوی واقعی و ورود عمومی نیز بدون mock و بدون ارسال OTP کنترل شدند. دیتابیس و سرویس تماس بدون restart یا تغییر باقی ماندند. شناسه image و rollback مستقل ورود در `docs/LAST_DEPLOYMENT.md` است.
